...
// like the old app/ui:
// - `bun run dev`   -> `vite build --watch` continuously rebuilds `dist`.
//   Tauri's built-in dev server serves `dist` (frontendDist) and hot-reloads
//   the webview whenever those files change, so edits appear live while
//   `cargo tauri dev` keeps Rust in dev too.
// - `bun run build` -> a clean production build into the same `dist` folder,
