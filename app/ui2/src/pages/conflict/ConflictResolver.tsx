    ...
    <section class="page active" id="page-conflicts">
      ...
      <div class="conflict-layout">
        <aside class="conflict-files">
          {info.files.length === 0 ? (
            <div class="conflict-empty" style={{ padding: "24px" }}>
            ...
            info.files.map((f) => {
              ...
              return (
                <div
                  class={`cfl-row${f === conflictActiveFile.value ? " active" : ""}`}
                  key={f}
                  onClick={() => (conflictActiveFile.value = f)}
                >
                  <span class="cfl-ico" dangerouslySetInnerHTML={{ __html: WARN }} />
                  <span class="cfl-name" title={f}>
                    {dir ? <span class="cfl-dir">{dir}</span> : null}
                    {fname}
                  </span>
                  <span class="cfl-badge">C</span>
                </div>
              );
            })
          )}
        </aside>
        <section class="conflict-main">
...
function ConflictFile({ path }: { path: string }) {
  ...
  if (error) return <div class="conflict-empty">{error}</div>;
  if (binary) {
    return (
      <>
        <div class="cv-bar">
          <span class="cv-path">{path}</span>
  ...
  if (!segments) return <div class="conflict-empty">Loading…</div>;
...
function CodeLines({ lines, path }: { lines: string[]; path: string }) {
  const html = lines.map((l) => `<div class="cv-line">${l === "" ? "&nbsp;" : hlConflict(l, path)}</div>`).join("");
...
function ConflictBlock({
  ...
  if (seg.choice) {
    ...
    return (
      <div class="cv-block">
        <div class="cv-side-label resolved">
          <span>✓ {label}</span>
          <span class="cv-side-actions">
            <span class="cv-undo" onClick={() => onPick(idx, null)}>
              Undo
            </span>
          </span>
        </div>
        <div class="cv-side-lines">
  ...
  return (
    <div class="cv-block">
      <div class="cv-side-label ours">
        <span>Current change · {ours}</span>
        <span class="cv-side-actions">
          <button class="cv-mini ours" type="button" onClick={() => onPick(idx, "ours")}>
            Accept current
          </button>
          <button class="cv-mini" type="button" onClick={() => onPick(idx, "both")}>
            Accept both
          </button>
        </span>
      </div>
      <div class="cv-side-lines ours">
        <CodeLines lines={seg.ours} path={path} />
      </div>
      <div class="cv-sep" />
      <div class="cv-side-label theirs">
        <span>Incoming change · {theirs}</span>
        <span class="cv-side-actions">
          <button class="cv-mini theirs" type="button" onClick={() => onPick(idx, "theirs")}>
            Accept incoming
          </button>
        </span>
      </div>
      <div class="cv-side-lines theirs">
